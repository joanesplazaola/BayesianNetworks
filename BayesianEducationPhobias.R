...
for (idx in 1:(length(functions))){
  scores <- functions[[idx]](train_data_educ_ph, score = args[idx]) %T>%
...
net.grain <- grain(as(amat(net), "graphNEL"), data = as.data.frame(data_educ_ph), smooth = 0.001)
...
row.names(phobiaEducation.Table) <-
  c('Marginal', 'Primary pupil', 'Primary school', 'Secondary school', 'College/Bachelor', 'Master', 'Doctor')
